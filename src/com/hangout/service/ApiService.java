...
//						public void run() {
							try {
								...
								ApiService.this.sendMessage(MSG_API_IO_EXCEPTION,MSG_API_GET_SELF);
							} catch (TokenExpiredException e) {
								ApiService.this.sendMessage(MSG_API_NEED_AUTHENTICATION,null);
							}
						//}
...
//                	}.start();
                ...
                case MSG_API_GET_UPCOMMING_EVENTS:
                	...
                	new Thread() {
						...
						public void run() {
							try {
								Events events = getHangoutApplication().getApi().getUpcommingEvents(memberId);
								ApiService.this.sendMessage(MSG_API_GET_SELF, events);
							} catch (IOException e) {
								ApiService.this.sendMessage(MSG_API_IO_EXCEPTION,MSG_API_GET_UPCOMMING_EVENTS);
							} catch (TokenExpiredException e) {
								ApiService.this.sendMessage(MSG_API_NEED_AUTHENTICATION,null);
							}
						}
                ...
                case MSG_API_GET_GROUPS:
                	...
                	new Thread() {
						...
						public void run() {
	                		try {
	                			Groups groups = getHangoutApplication().getApi().getGroups(memberId);
	                			ApiService.this.sendMessage(MSG_API_RECEIVED_GROUPS,groups);
	                		} catch(IOException e) {
	                			ApiService.this.sendMessage(MSG_API_IO_EXCEPTION,MSG_API_GET_GROUPS);
	                		} catch (TokenExpiredException e) {
	                			ApiService.this.sendMessage(MSG_API_NEED_AUTHENTICATION,null);
							}
						}
                ...
                case MSG_API_MEETUP_TOKEN_CHANGED:
                	...
                	getHangoutApplication().getConsumer().setTokenWithSecret(storage.getMeetupToken(), storage.getMeetupTokenSecret());
                default:
